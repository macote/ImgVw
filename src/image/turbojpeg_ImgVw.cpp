...
// Based on libjpeg-turbo 1.5.2
// (https://github.com/libjpeg-turbo/libjpeg-turbo/commit/e5c1613ccdfeffcd060fd94248b7c8ac7c0cfb0f)
...
getSubsamp(j_decompress_ptr dinfo)
{
    ...
    for (i = 0; i < NUMSUBOPT; i++)
    {
        if (dinfo->num_components == pixelsize[i] ||
            ((dinfo->jpeg_color_space == JCS_YCCK || dinfo->jpeg_color_space == JCS_CMYK) && pixelsize[i] == 3 &&
             dinfo->num_components == 4))
        {
            ...
            if (dinfo->comp_info[0].h_samp_factor == 2 && dinfo->comp_info[0].v_samp_factor == 2 &&
...
METHODDEF(int)
setDecompDefaults(struct jpeg_decompress_struct* dinfo, int pixelFormat, int flags)
{
    int retval = 0;
...
int Destroy(tjhandle handle)
{
    ...
    if (setjmp(inst->jerr.setjmp_buffer))
        return -1;
    if (inst->init & COMPRESS)
        jpeg_destroy_compress(cinfo);
    if (inst->init & DECOMPRESS)
...
int DecompressHeader(tjhandle handle, const unsigned char* jpegBuf, unsigned long jpegSize, int* width, int* height,
                     int* jpegSubsamp, int* jpegColorspace, int flags)
{
    int retval = 0;
    ...
    if (jpegBuf == nullptr || jpegSize <= 0 || width == nullptr || height == nullptr || jpegSubsamp == nullptr ||
        jpegColorspace == nullptr)
...
int Decompress(tjhandle handle, const unsigned char* jpegBuf, unsigned long jpegSize, unsigned char* dstBuf, int width,
               int pitch, int height, int pixelFormat, int flags)
{
    ...
    if (jpegBuf == nullptr || jpegSize <= 0 || dstBuf == nullptr || width < 0 || pitch < 0 || height < 0 ||
...
marker_is_exif(jpeg_saved_marker_ptr marker)
{
    return marker->marker == EXIF_MARKER && marker->data_length >= EXIF_OVERHEAD_LEN &&
           ...
           GETJOCTET(marker->data[0]) == 0x45 && GETJOCTET(marker->data[1]) == 0x78 &&
           GETJOCTET(marker->data[2]) == 0x69 && GETJOCTET(marker->data[3]) == 0x66 &&
           GETJOCTET(marker->data[4]) == 0x0 && GETJOCTET(marker->data[5]) == 0x0;
}
